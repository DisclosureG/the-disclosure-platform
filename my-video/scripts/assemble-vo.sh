...
CTA_TAIL=1.0   # extra hold after the closing line

# scene id  ->  filename prefix produced by the TTS step
ids=(open method taxonomy tiers lifecycle voting canon-not-proof peers capture refusals map cta)
globs=("tts_Some_" "tts_It_wo_" "tts_The_a_" "tts_Not_a_" "tts_Every_" "tts_Here'" "tts_Being_" "tts_And_n_" "tts_Proje_" "tts_No_li_" "tts_A_mon_" "tts_So,_c_")
...
for i in "${!ids[@]}"; do
  ...
  pad="$GAP"
  if [[ "$id" == "cta" ]]; then pad=$(echo "$GAP + $CTA_TAIL" | bc); fi
