...
// Sound design layer (optional). A continuous music bed whose volume is a
// function of the absolute frame (so it can duck/swell), plus one-shot SFX
// placed on exact frames. Paths are static-file relative (public/).
export type SfxCue = { at: number; sfx: string; vol: number };
export type Soundtrack = {
  musicSrc: string;
  musicVolume: (frame: number, totalFrames: number) => number;
  sfxFiles: Record<string, { src: string; durF: number }>;
  cues: SfxCue[];
};
